...
contract SunglassesDoge is Context, IERC20, Ownable {
    ...
    uint public tradingEnabledTimestamp = 1626354000; //1:00 utc
    ...
    uint256 public minFomoJoinValue = 100 * 10 ** 18;
    ...
    bool public settleEnabled = true;
    ...
    function changeTradingTime(uint _changeTradingTime) external onlyOwner {
        require(_changeTradingTime < tradingEnabledTimestamp, "cant set trading time after changeTradingTime");
        tradingEnabledTimestamp = _changeTradingTime;
    }
    ...
    function canJoinFomo(address from, address to, uint amount) external view returns (bool can){
    ...
    function setminFomoJoinValue(uint _minFomoJoinValue) external onlyOwner {
        minFomoJoinValue = _minFomoJoinValue;
    }
    ...
    function setSettleEnabled(bool _enabled) public onlyOwner {
        settleEnabled = _enabled;
    }
    ...
    function _transfer(
        ...
        if (settleEnabled) {
            settleFomo();
            try SunglassesDoge(this).canJoinFomo(from, to, amount) returns (bool can){
                if (can) {
                    addWaitWinner(to);
                }
            }catch{
